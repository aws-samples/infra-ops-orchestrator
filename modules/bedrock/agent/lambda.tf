...
resource "aws_sqs_queue" "orchestrator_container_lambda_dql" {
  name              = "orchestrator-container-lambda-dlq-${var.app_name}-${var.env_name}"
...
resource "aws_lambda_function" "orchestrator_container_lambda" {
  function_name                  = "orchestrator-container-lambda-${var.app_name}-${var.env_name}"
  ...
  memory_size  = 1024
  layers = [
    "arn:aws:lambda:${local.region}:754028404239:layer:AWSLambdaPowertoolsPythonV2-acme-prod:1"
  ]
  tracing_config {
    mode = "Active"
  }
  dead_letter_config {
    target_arn = aws_sqs_queue.orchestrator_container_lambda_dql.arn
  }
  vpc_config {
...
resource "aws_lambda_permission" "allow_bedrock_lambda_invoke" {
  ...
  function_name  = aws_lambda_function.orchestrator_container_lambda.function_name
...
resource "aws_lambda_alias" "scale_up_lambda_alias" {
  name             = "provisioned-${aws_lambda_function.orchestrator_container_lambda.function_name}"
  description      = "Alias for provisioned instances of ${aws_lambda_function.orchestrator_container_lambda.function_name}"
  function_name    = aws_lambda_function.orchestrator_container_lambda.function_name
  function_version = aws_lambda_function.orchestrator_container_lambda.version
}

# Provisioned concurrency set at 3
...
resource "null_resource" "lambda_code_update" {
  depends_on = [aws_lambda_function.orchestrator_container_lambda]

  provisioner "local-exec" {
    command = "aws lambda update-function-code --function-name ${aws_lambda_function.orchestrator_container_lambda.function_name} --s3-bucket ${var.code_base_bucket} --s3-key ${var.code_base_zip}  --region ${local.region}"
  }
...
resource "aws_cloudwatch_log_group" "lambda_log_group" {
  name              = "/aws/lambda/${aws_lambda_function.orchestrator_container_lambda.function_name}"
  retention_in_days = var.cloudwatch_log_group_retention
  kms_key_id        = var.kms_key_id
}

resource "aws_cloudwatch_log_stream" "lambda_log_stream" {
  name           = "${aws_lambda_function.orchestrator_container_lambda.function_name}-cw-stream"
