...
resource "aws_bedrockagent_agent_action_group" "bedrock_agent_actiongroup" {
  ...
  action_group_executor {
    lambda = aws_lambda_function.orchestrator_container_lambda.arn
  }
  api_schema {
...
resource "null_resource" "agent_prepare" {
  ...
  provisioner "local-exec" {
    command = "aws bedrock-agent prepare-agent --agent-id ${aws_bedrockagent_agent.bedrock_agent.id} --region us-west-2"
  }
  depends_on = [
