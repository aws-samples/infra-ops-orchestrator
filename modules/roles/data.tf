...
data "aws_iam_policy_document" "agent_lambda_role_policy" {
  statement {
    ...
    resources = [
      "arn:aws:logs:${local.region}:${local.account_id}:log-group:/aws/lambda/orchestrator-container-lambda-${var.app_name}-${var.env_name}"
    ]
  }
  ...
  statement {
    ...
    resources = ["arn:aws:sqs:${local.region}:${local.account_id}:orchestrator-container-lambda-dlq-${var.app_name}-${var.env_name}"]
  }
